...
export default function MetricCard({ icon, iconBg = 'bg-info-bg', iconColor = 'text-info', value, unit, label, trend, tone = 'default' }) {
  ...
  return (
    <div className={`rounded-lg p-card-padding border shadow-l1 ${isDanger ? 'bg-danger-bg border-[#f3c9c4]' : 'bg-card-white border-stroke'}`}>
      ...
      <p className={`text-headline-lg leading-8 ${isDanger ? 'text-[#93000a]' : 'text-on-surface'}`}>
        {value}
